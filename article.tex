...
Furthermore, our model's accuracy can be obtained in two different aspects. The first one would be the correctness of each time-slice prediction ($\overline{C}_{i,t}$), while the second, is the accuracy of disease diagnosis ($\overline{C}_i$), and we are mostly interested in the later. Both of the mentioned statistics can be calculated using Equation \eqref{eq:meanCorrectness}.
...
\overline{C_{i,t}} &=& {{\sum_{i=1}^{m} \sum_{t=1}^{T (=198)} C_{i,t} } \over {m \times T} } \\ 
\overline{C_i} &=& {{\sum_{i=1}^{m} C_i } \over {m } }  \nonumber 
\end{eqnarray}
...
\begin{table}
	...
	\begin{tabular}{ c | c | c }
		\hline
		$i$ & $\overline{C_{i,t}}\;(\%)$ & $\overline{C_i}\;(\%)$ \\ 
