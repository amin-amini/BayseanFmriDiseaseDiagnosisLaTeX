...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\STATE \textbullet~Shuffle scans
		\STATE \textbullet~Put 6 scans (3 group each) aside as test data
		\STATE \textbullet~Split time-slices of data and shuffle them
		\STATE \textbullet~Divide train and validation data to 50 chunks
		\FORALL{50 chunks} 
		\STATE train network (Fig. \ref{fig:cnnArch}) 
		\STATE \hskip1.0em \textbullet~First
		\ENDFOR
		%\IF {$i\geq 5$} 
		%\STATE $i\gets i-1$
		%\ELSE
