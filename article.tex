...
\begin{abstract}
The goal of resting-state fMRI is to investigate the brain's functional connections as an indicator of synchronous neural activity.
However different images of patients' brains regardless of time-based information, may contain enough data to investigate their brains.
We propose a novel method to automatically extract non-time sequence-based information using a Bayesian deep learning algorithm based on a convolutional neural network.
Instead of using some predefined points of interest, we use the whole data in the training phase and those points which do not contain related information about the disease, will be ignored automatically.
...
Each fMRI scan of patients will be divided into {\it t} different 3D images making dataset bigger and calculations simpler.
To determine if a person is a patient, we test {\it t} fMRI slices and get {\it t} different results which leads to a fraction (probability) of how unhealthy the brain is. Experimental results confirm the efficiency of the method and its outperformance.
\end{abstract}


\begin{highlights}
\item We introduce a novel Bayesian Deep Convolutional Network for Parkinson's disease diagnosis.
\item The proposed method does not make assumptions about the disease or the dataset, making it a possible solution for other diseases.
\item Our model only relies on spatial information and ignores temporal information in the fMRI.
\item Computational costs of the proposed method is negligible compared to other existing methods. As a result, it is a widely applicable algorithm for low-end processors.
\item The experimental results confirm the high efficiency of the proposed method.
\end{highlights}
...
Parkinson's disease is a long-term degenerative disorder of the central nervous system that mainly affects the motor system. As the disease worsens, non-motor symptoms become more common. The symptoms usually emerge slowly. Early in the disease, the most obvious symptoms are shaking, rigidity, slowness of movement, and difficulty with walking. Thinking and behavioral problems may also occur. Recently, resting-state fMRI has become an especially popular area of research, and, as a result, the publication of related articles has grown exponentially \citep{Murphy2013}. The goal of resting-state fMRI is to use the common variance of the fMRI blood oxygenation level dependent (BOLD) signals in different regions of the brain as an indicator of synchronous neural activity \citep{GomezLaberge2011}\citep{Oikonomou2010}. The assumption is that the temporal similarity between the BOLD signals in each region demonstrates that they are in constant communication with one another and thus form a functional network \citep{Calhoun2016}.

There are many ways to define network nodes from fMRI; nodes are often defined as spatial regions of interest, for example, as obtained from fMRI activation or from brain atlases \citep{Arribas2010}\citep{Calhoun2012}. Alternatively, parcellation via a data-driven clustering of the FMRI data itself (e.g., hierarchical clustering or independent component analysis) can be run to define clusters or components (spatial maps with associated time-slices), which can be considered network nodes \citep{Smith2012}. Furthermore, we are curious to find out if fMRI scans carry enough information without taking their temporal connectivities into account. In other words, does spatial information carry enough data to diagnose Parkinson's disease patients?

The majority of fMRI disease diagnosis algorithms are based on temporal information and functional connectivities in some regions of interest (ROIs)\citep{Adali2014}. For instance, Shen et. al had proposed a method based on functional connectivities in some ROIs, which enabled the differentiation of schizophrenic patients from healthy control with an accuracy of $81.3\%$ \citep{Shen2014}. Another powerful model is the hybrid method of static and dynamic resting-state fMRI connectivities proposed by Rashid et. al. The mentioned method uses structural and functional connectivities coherently to classify schizophrenia and bipolar patients accurately ($=89\%$) \citep{Rashid2016}. In contrast to that, there are some powerful methods such as Sarraf and Tofighi's deep 2D convolutional neural network algorithm which can diagnose Alzheimer's disease patients using their proposed adopted LeNet-5 network as accurate as $96.86\%$ \citep{Sarraf2016}. Moreover, Sarraf and Tofighi's method does not make assumptions about the disease, makes it a possible solution for Parkinson's disease diagnosis.

Sarraf and Tofighi's 2D CNN method is one of the best candidates for Parkinson's disease classification. However, it has some downsides which make it unable to do so. First of all, it needs a huge amount of training samples to learn as expected. Secondly, we have to omit some participants to prevent overfitting \citep{Sarraf2016}. Also, this method consumes a lot of hardware resources and the training phase takes hours to complete.

Moreover, there are some other methods, such as the method proposed by Riaz et. al. which is a hybrid approach based on functional connectivity and deep learning to classify ADHD patients. This algorithm divides functional connectivities to different clusters and employs an Elastic Net to select the most discriminant features among the connectivities and the phenotypic information. Finally, a support vector machine will be trained as the classifier \citep{Riaz2020}. This method has shown an acceptable performance on some datasets and can be used for other diseases as well. Unlike our proposed method, this one is still focusing on time-based data such as functional connectivity.

Another eye-catching research is "Detecting Alzheimer’s disease based on 4D fMRI" by Li et. al. This method is empowered by coupling 3D-CNN with long short-term memory (LSTM) together. Similar to the latter algorithm, this one also tries to take advantage of both temporal and structural information inside the 4D images of fMRI. In brief, the mentioned method feeds each time-slice to a different CNNs to extract spatial features. Then the feature maps will be put into an LSTM to capture time-varying features. Subsequently, the output of the last layer will enter a fully-connected layer to do the classification \citep{Li2020}. Although this method is well-engineered, it still utilizes both spatial and temporal information. As mentioned earlier, our method will focus only on spatial data to find out about the potential of it in the task of classification.

"Spatio-temporal deep learning method for ADHD fMRI classification" is another deep learning model by Mao et. al. which worth mentioning. This algorithm is an ensemble method based on three different 3D-CNN and LSTM classifiers. The first network uses a single frame approach and a global average pooling in the end. The second network uses multi-frame CNNs coupled with a max-pooling layer and, the last part implements LSTM layers before pooling. In the end, all of these outputs are fed to a 4D-CNN to train on extracted features \citep{Mao2019}. This method uses both temporal and spatial features together without any assumptions on the data or hand-crafted features and performs astonishing.

In this study, we are trying to build a method based on Bayesian deep learning and spatial resting-state fMRI data to diagnose Parkinson's disease accurately. Our proposed method takes advantage of hidden spatial connectivities using Three-dimensional Convolutional Neural Networks (3D CNN) while keeping calculations affordable; thanks to the Bayesian nature of the proposed network. Division of scans to {\it t} different images, on the other hand, speeds up the training phase and keeps our model as simple as possible; and as a result, our algorithm can avoid overfitting. This technique prevents the network from searching for low-level connections among time-series as well as multiplication of the data. Correspondingly, the proposed method can evaluate each validation case {\it t} times to prevent overfitting effectively. Besides, we are trying to avoid any kind of assumptions and conditions about the disease (Parkinson's) and participants. Subsequently, the proposed method might be able to be used in case of other diseases as well. Also, if we can classify patients successfully, we can state that spatial fMRI data carries a lot of hidden information worth investigating in the future.
...
feature maps in the $(i-1)$th layer connected to the current feature map $w^{pqr}_{ijm}$ is the value at the position $(p,q,r)$ of the kernel connected to the $m$th feature map, and $P_i$, $Q_i$ and $R_i$ are the height, width and depth of the kernel, respectively \citep{Ji2013}.
...
\begin{table}
	...
	\resizebox{\linewidth}{!}{%
		\begin{tabular}{ | c | c | c | c | c | c | c | c | }
			\hline
			patient & $Y_i$ & $N_{i,hc}$ & $N_{i,p}$ & $C_{i,t} (\%)$ & $P_i$ & $\hat{Y}_i$ & $C_i (\%)$ \\  \hline
...
In this study, we showed that spatial information is powerful enough to diagnose Parkinson's disease. Moreover, we can try to implement a hybrid solution based on the proposed method that uses temporal connectivities as well. For instance, we can use functional connectivity based algorithms \citep{Lehmann2017}\citep{Leonardi2013}\citep{Liu2016}\citep{Liu2015}\citep{Ryali2012} or statistical/Bayesian based methods \citep{Ahmad2016}\citep{Oikonomou2013}\citep{Oikonomou2012}\citep{Power2012}\citep{Suk2016}. Ensembling can be done by training mentioned methods combined with Bayesian units or by adding their extracted features to the existing network.
